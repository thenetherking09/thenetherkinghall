document.addEventListener("DOMContentLoaded", () => {
    ...
    if(btn) btn.addEventListener("click", () => sendToTelegram('contact'));
});
...
async function sendToTelegram(type) {
    ...
    if (file) {
        // Enforce 4MB limit to prevent Base64 JSON payload from crashing Serverless Functions
        ...
        if (file.size > maxSize) {
            ...
            if (btn) {
                btn.innerText = "Submit to the NetherKing";
                btn.disabled = false;
            }
            return;
        }
        ...
        // Do not compress videos or GIFs (canvas will break them)
    ...
    try {
        ...
        if (status) {
            ...
            status.innerText = "Submitted successfully to the Realm.";
        }
